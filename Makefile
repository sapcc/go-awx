...
build-all: build/awx-go

build/awx-go: FORCE
	@env $(GO_BUILDENV) go build $(GO_BUILDFLAGS) -ldflags '-s -w $(GO_LDFLAGS)' -o build/awx-go .
...
install: FORCE build/awx-go
	install -d -m 0755 "$(DESTDIR)$(PREFIX)/bin"
	install -m 0755 build/awx-go "$(DESTDIR)$(PREFIX)/bin/awx-go"
...
goimports: FORCE
	@printf "\e[1;36m>> goimports -w -local https://github.com/sapcc/awx-go\e[0m\n"
	@goimports -w -local github.com/sapcc/go-awx $(patsubst $(shell awk '$$1 == "module" {print $$2}' go.mod)%,.%/*.go,$(shell go list ./...))
...
help: FORCE
	...
	@printf "  \e[36mbuild/awx-go\e[0m                 Build awx-go.\n"
